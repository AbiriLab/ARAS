...
Code for creating and saving the screens in phase 1
...
def get_screen(environment):
    ...
    plt.figure()
    plt.imshow(rgb)
    plt.title('128 x 128 image')
    plt.savefig("figures/image1.pdf")
    plt.show()
...
# this is a phase 2 model, but does not matter for reconstruction of scene
...
for i_episode in range(episode):
    ...
    plt.figure()
    plt.imshow(get_screen(environment=env).cpu().squeeze(0)[-1].numpy(),cmap='Greys',
            interpolation='none')
    plt.title('Preprocessed image 40 x 40')
    plt.savefig("figures/image2.pdf")
    plt.show()
