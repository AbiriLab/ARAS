...
Code for creating and saving the screens in phase 2
...
def get_screen(environment):
    ...
    plt.savefig("figures/phase/1/image1.pdf")
    plt.show()
...
for i_episode in range(episode):
    ...
    plt.imshow(get_screen(environment=env).cpu().squeeze(0)[-1].numpy(),cmap='Greys',
            interpolation='none')
    plt.title('Preprocessed image 128 x 128')
    plt.savefig("figures/image1_preprocessed.pdf")
    plt.show()
